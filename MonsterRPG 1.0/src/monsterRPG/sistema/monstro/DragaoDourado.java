...
public class DragaoDourado extends Monstro{
	...
	private void preencherHabilidades() {
		...
		this.getHabilidades().add(new Habilidades("Sopro elemental (de enfraquecimento)", "O dragão exala gás em um cone de 15 pés. Cada criatura nessa área precisa passar em um teste de resistência de Força de dificuldade 13, caso contrário a criatura tem desvantagem em rolagens de ataques baseados em Força, testes de Força e testes de resistência de Força por 1 minuto. A criatura pode repetir o teste de resistência no final de cada um de seus turnos, acabando com o efeito em um sucesso."));
